...
const deleteProduct = async (req, res) => {
    ...
    try {
        ...
        const deleteImageFiles = product.images.map(async (imageName) => {
            const imagePath = path.join('src/public', 'images', imageName)
            try {
                await fs.unlink(imagePath)
            } catch (error) {
                console.error(`Không thể xóa ảnh ${imageName}:`, err.message);
            }
        })
